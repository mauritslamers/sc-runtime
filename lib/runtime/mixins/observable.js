...
SC.Observable = {
  ...
  */
  set: function(key, value) {
    ...
    if(process && process.nextTick){ // force the runloop to run when set is used
      if(SC.RunLoop && SC.RunLoop.isRunLoopInProgress && SC.RunLoop.isRunLoopInProgress()){ // only trigger when not already in progress
        SC.RunLoop.begin();
